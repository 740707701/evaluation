...
import headerNav from '../components/HeaderNav'
  export default {
    ...
    methods: {
      ...
      //单选
      checkOne(e, id){
        console.log(e.target, e.target.checked, id)
        var checkOneItem = e.target
        if(checkOneItem.checked){
          if(this.checkedIdList.indexOf(id) == -1){
            this.checkedIdList.push(id)
          }
          if(this.checkedIdList.length == this.cepingList.length){
            this.isCheckAll = true
          }
        }else {
          this.isCheckAll = false
