        ...
        <el-col :span="6" class="right-content">
          <div class="grid-content">
            <div class="hr-tag">
              ...
              <el-select size="small" v-model="tag" filterable :filter-method="filterTag(tag)" placeholder="请选择" class="tag">
                <el-option
                  v-for="item in tags"
                  :key="item.value"
                  :label="item.label"
              ...
              <div class="tag-box" v-for="item in filterTags" :key="item.name">
...
export default {
      ...
      allTagList: [
        {
          type: `base`,
          name: `姓名`,
          msg: `姓名是HR发出面试邀约和offer所需要的重要信息，不可有误。`
        },
        {
          type: `base`,
          name: `性别`,
          msg: `部分工种对性别有要求，但不会明写在岗位描述中，（因劳动法有规定不能有性别歧视）但HR会根据面试者的性别去进行相应判断，
                 如果不适合则不会发出面试邀约，节省彼此的时间。`
        },
        {
          type: `base`,
          name: `出生日期(年龄)`,
          msg: `1.根据年龄判断工作年限是否差异较大 
                2.不乏有企业老板或者部门领导对面试者的星座、年龄等有所要求。如果硬性要求不符合，可通过简历的初筛节省双方的时间。`
        },
        {
          type: `base`,
          name: `电话号码`,
          msg: `1.电话号码是用人单位联系面试者的最常用工具，保持电话畅通才不会漏过每一个面试邀约。
        ...
        },
        {
          type: `base`,
          name: `联系邮箱`,
          msg: `1.邮箱为用人单位（企业）发送面试信息、录取offer等文件资料的重要通讯工具，需要保证邮箱状态正常。
                  2.一般企业不会因为你的邮箱为QQ邮箱而放弃对你的面试邀请，所以不必为了面试专门申请邮箱，只需要保证邮箱常用且能正常收发邮件即可。`
        },
        {
          type: `base`,
          name: `现居地`,
          msg: `根据求职者现居地判断到公司路程远近，一般在北上深这样的城市单程1小时内是求职者可以接受的路程范围，
                  如果太远则求职者可能会不稳定。电话沟通时也会问求职者是自住房还是租房，如果路程较远是否考虑公司附近租房等。`
        },
        {
          type: `base`,
          name: `婚姻状况`,
          msg: `1.女性求职者在适婚年龄会涉及到婚假、产假等带薪假期，会对企业造成一定的影响，包括企业的用人成本、工作的持续程度等
                  2.已婚者通常没有未婚者在工作上投入的精力多，事业家庭兼顾的情况势必会有侧重。`
        },
        {
          type: `work`,
          name: `工作经验`,
          msg: `1.一般情况下正式工作是以拿到毕业证起开始计算时间的，未取得毕业证学位证之前的实习或工作不计入工作经验中。
                  2.实习期间无法确保该员工的稳定性，一般实习生所承担的工作也不会是含金量高的内容，所以全职工作经验与实习/兼职通常不放在一起衡量。`
        },
        {
          type: `expect`,
          name: `期望薪资`,
          msg: `1.期望薪资是求职者心里对该岗位及公司所能给到的待遇的一个预估及对自我情况的判断。
        ...
        },
        {
          type: `expect`,
          name: `到岗时间`,
          msg: `1.若用人单位急需用人，则希望求职者立即到岗，筛选简历会看求职者的到岗时间来进行邀约判断，若选择到岗时间的周期较长，可能在简历初筛阶段就被淘汰掉了。
        ...
        },
        {
          type: `expect`,
          name: `工作类型`,
          msg: `1.全职一般指求职者为满足该岗位需求的可签署劳动合同的自然人，毕业生已取得学历证、学位证，且人事档案、组织关系等根据招聘方需求可选择转入该公司，公司为全职员工依法缴纳社保公积金等，正常情况下员工要保证每周不低于40小时的上班时间。
  ...
  methods: {
    ...
    postResume: function() {
      ...
      this.$store.dispatch('SUBMIT_RESUME', this.postInfo).then(res => {
    ...
    filterTag: function(value){
      ...
      if(!value){ 
        this.filterTags = this.allTagList
        return 
      }else {
        ...
        let filterTags = this.allTagList.filter((item) => {
          return item.type == type
        })
        // this.filterTags = filterTags //会报错
